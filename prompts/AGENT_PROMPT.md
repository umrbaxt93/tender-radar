...
## Current task
Review the supplied scaffold/spec snapshot only. Return a concise English Markdown report with:
1. Concrete specification inconsistencies or missing evidence.
2. The next bounded implementation task, prerequisites and acceptance checks.
3. Any source-access, cloud-runtime or cost blocker.
Do not implement anything, invoke tools, request secrets, or output application code.
Do not claim to inspect files or run tests: the complete allowed context is supplied in this prompt.
Keep the response below 700 words. Do not infer completion from earlier assistant promises.
